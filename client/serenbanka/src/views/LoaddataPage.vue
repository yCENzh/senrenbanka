...
const loaddataImgFiles = import.meta.glob('../assets/static/loaddata/*.png', { eager: true, query: '?url', import: 'default' });
const soundFiles = import.meta.glob('../assets/static/sound/*.ogg', { eager: true, query: '?url', import: 'default' });
const loaddataImg = (name) => loaddataImgFiles[`../assets/static/loaddata/${name}`];
...
const baseImages = Array.from({ length: 17 }, (_, i) =>
    loaddataImg(`baseImages/${i}.png`)
);

// 悬停图片路径数组
const hoverImages = Array.from({ length: 17 }, (_, i) =>
    loaddataImg(`hoverImages/${i}.png`)
);

// 点击图片路径数组
const activeImages = Array.from({ length: 17 }, (_, i) =>
    loaddataImg(`activeImages/${i}.png`)
);
...
const ensureClick = async (index) => {          //此处应该同时根据mode、选择的blockChoicedID1,blockChoicedID2进行判断。
    if (index === 0) {
        eventBus.$emit('set-sound2', soundUrl('buttonclick-save.ogg'));
